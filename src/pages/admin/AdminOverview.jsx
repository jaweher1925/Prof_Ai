  ...
  return (
    <div className="max-w-6xl mx-auto px-8 py-10">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        ...
        <p className="text-sm text-slate-400 mt-1">Platform-wide usage across every professor account.</p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5 mt-6">
        ...
        <TiltCard className="lg:col-span-2 rounded-2xl border border-white/[0.08] bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-xl p-6 shadow-xl shadow-black/30">
          <div style={{ transform: 'translateZ(24px)' }}>
            <div className="flex items-center gap-2 mb-5">
              <ShieldCheck className="w-4 h-4 text-emerald-400" />
              <h2 className="text-sm font-semibold text-white">HeyGen engine usage</h2>
            </div>
            ...
            {heygen.avatarIVRenders > 0 && (
              <div className="flex items-start gap-2 mt-5 px-3 py-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20">
                ...
                <p className="text-xs text-amber-200/80">{heygen.avatarIVRenders} scene(s) rendered on the costlier engine because their avatar doesn't support Avatar III.</p>
              </div>
            )}
            <p className="text-[11px] text-slate-500 mt-4 leading-relaxed">{heygen.note}</p>
          </div>
        </TiltCard>

        <TiltCard className="lg:col-span-3 rounded-2xl border border-white/[0.08] bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-xl p-6 shadow-xl shadow-black/30">
          <div style={{ transform: 'translateZ(24px)' }}>
            <h2 className="text-sm font-semibold text-white mb-5">Recently updated projects</h2>
            <div className="space-y-1">
              {recentProjects.length === 0 && <p className="text-sm text-slate-500">No projects yet.</p>}
              {recentProjects.map((p) => (
                <div key={p.id} className="flex items-center justify-between px-3 py-2.5 rounded-lg hover:bg-white/[0.03] transition-colors">
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    <span className="text-[11px] text-slate-500">{new Date(p.updatedAt).toLocaleDateString()}</span>
                  </div>
                </div>
              ))}
            </div>
