...
export default function ScriptsPanel({ project, onUpdate, onContinue }) {
  ...
  const [editingScene, setEditingScene] = useState(null)   // { scriptId, sceneIndex, text }
  ...
  // Save edited scene back into the script's sections JSON
  const handleSaveScene = async (script, sceneIndex) => {
    if (!editingScene) return
    setSavingScene(true)
    try {
      const sections = JSON.parse(script.sections || '[]')
      sections[sceneIndex] = { ...sections[sceneIndex], script_content: editingScene.text }
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      <div className="mb-6 p-4 rounded-xl bg-slate-900/40 border border-white/[0.06]">
        ...
        <p className="text-xs text-slate-500 mb-3">
          Writes 8 scenes per video (6 min total). You can edit any scene before approving.
        </p>
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          {sections.map((scene, i) => {
                            ...
                            const slideContent = (() => { try { return typeof scene.slide_content === 'string' ? JSON.parse(scene.slide_content) : (scene.slide_content || {}) } catch { return {} } })()
                            const slideBullets = slideContent.blocks?.[0]?.items || []
                            return (
                              <div key={i} className="rounded-xl bg-slate-800/50 border border-white/[0.06] overflow-hidden">
                                {/* Scene header */}
                                <div className="flex items-center justify-between px-3 py-2 bg-slate-900/30 border-b border-white/[0.04]">
                                  ...
                                  {!isEditing ? (
                                    <button
                                      onClick={() => setEditingScene({ scriptId: script.id, sceneIndex: i, text: scene.script_content })}
                                    ...
                                    <div className="flex items-center gap-2">
                                      <button onClick={() => handleSaveScene(script, i)} disabled={savingScene}
                                ...
                                <div className="grid grid-cols-2 divide-x divide-white/[0.04]">
                                  ...
                                  <div className="p-3">
                                    ...
                                    {isEditing ? (
                                      ...
                                      <p className="text-xs text-slate-400 leading-relaxed">{scene.script_content}</p>
                                    )}
                                  </div>
                                  ...
                                  <div className="p-3">
                                    ...
                                    {slideContent.title ? (
                                      <div className="space-y-1.5">
                                        <p className="text-xs font-semibold text-white leading-snug">{slideContent.title}</p>
                                        {slideContent.subtitle && (
                                          <p className="text-[11px] text-slate-400 italic leading-snug">{slideContent.subtitle}</p>
                                        )}
