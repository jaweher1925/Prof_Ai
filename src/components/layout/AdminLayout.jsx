...
import { LayoutGrid, Users, FolderKanban, Settings, LogOut, ArrowLeft } from 'lucide-react'
...
/**
 * Admin console shell — deliberately distinct from the professor AppLayout/
 * Sidebar (always-dark slate, not theme-following) so it's visually obvious
 * at a glance which "mode" of the app you're in. Nav rail carries the same
 * real-3D language as TiltCard (perspective + depth via layered shadows/
 * translateZ-style elevation on hover) rather than reusing TiltCard itself,
 * since a full-height rail tilting toward the cursor would be disorienting
 * for navigation — the 3D here shows up as raised/pressed nav pills and the
 * stat cards on each admin page (see AdminOverview.jsx's StatCard).
 */
export default function AdminLayout({ children }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-950">
      <aside className="w-64 flex-shrink-0 h-screen flex flex-col bg-slate-950 border-r border-white/[0.06] relative overflow-hidden">
        ...
        <div className="relative px-5 py-5 border-b border-white/[0.06]">
          ...
          >
            <LogoBadge size="w-9 h-9" tone="white" />
            <div className="min-w-0">
              <p className="text-sm font-bold text-white leading-none truncate">ProfAI</p>
              <p className="text-[10px] text-indigo-300/70 tracking-[0.16em] uppercase mt-1 font-semibold">
                Admin Console
              </p>
            </div>
          </button>
        ...
        <div className="relative border-t border-white/[0.06] p-3 space-y-1">
          <button
            ...
            className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm text-slate-400 hover:bg-white/[0.04] hover:text-white transition-all duration-150"
          >
          ...
          <div className="flex items-center gap-3 px-3.5 py-2.5 rounded-xl bg-white/[0.03]">
            ...
            <button
              ...
              title="Sign out"
              className="flex-shrink-0 w-6 h-6 rounded-md flex items-center justify-center text-slate-500 hover:bg-rose-500/10 hover:text-rose-400 transition-colors"
            >
