...
export async function renderVoiceOnlyVideo(opts: {
  ...
  // ── HyperFrames animated video background path ────────────────────────────
  // When the scene's visualAssetUrl is an .mp4 (generated by HyperFrames), use
  // it as a real video background instead of a looped still image — no motion
  // filter needed (the HTML animation is already baked in), just mux with audio.
  const rawBgUrl  = opts.imageUrl || ''
  const isVideoBg = /\.mp4$/i.test(rawBgUrl)
  if (isVideoBg) {
    const videoBgPath = localPathFromUploadUrl(rawBgUrl)
    if (!videoBgPath) throw new Error('Could not locate the HyperFrames video file on disk.')
...
export async function compositeAvatarOverlay(opts: {
  ...
  // Avatar background transparency control — NEW FEATURE
  // "transparent" = alpha channel, no background fill
  // "color" = solid color fill (default, uses avatarBgColor)
  avatarBgType?: 'color' | 'transparent'
  avatarBgColor?: string | null
}): Promise<string> {
  const audioPath = localPathFromUploadUrl(opts.audioUrl)
  if (!audioPath) throw new Error('Could not locate the audio file on disk for this scene.')

  // ── HyperFrames animated video background ─────────────────────────────────
  const isVideoBg = /\.mp4$/i.test(opts.slideImageUrl || '')
  if (isVideoBg) {
    const videoBgPath = localPathFromUploadUrl(opts.slideImageUrl)
    if (!videoBgPath) throw new Error('Could not locate the HyperFrames slide video on disk.')

    // Download avatar clip to a temp file
    ...
    try {
      await runFfmpeg([
        '-y',
        '-i', videoBgPath,      // [0] HyperFrames animated slide
        '-i', avatarTmpPath,    // [1] HeyGen avatar clip
        '-i', audioPath,        // [2] TTS audio
        '-filter_complex',
          `[0:v]scale=${OUT_W}:${OUT_H}:force_original_aspect_ratio=decrease,` +
          `pad=${OUT_W}:${OUT_H}:(ow-iw)/2:(oh-ih)/2[bg];` +
          `[1:v]${avaFilter}[ava];` +
      ...
      try { unlinkSync(avatarTmpPath) } catch { /* best-effort */ }
    }
    return `/api/uploads/${outName}`
  }
  ...
  // FEATURE: Avatar background transparency support
  // When avatarBgType='transparent', avatar PiP has alpha channel (shows slide behind)
  // When avatarBgType='color', avatar has solid color background
  const useTransparent = opts.avatarBgType === 'transparent'
  ...
  // Avatar filter: optionally add transparency support
  let avaFilter = useCircle
    ? `scale=${pipSize}:${pipSize}:force_original_aspect_ratio=increase,crop=${pipSize}:${pipSize},` +
      `format=yuva420p,geq=lum='lum(X,Y)':a='if(lte(pow(X-${pipSize / 2},2)+pow(Y-${pipSize / 2},2),pow(${radiusPx},2)),255,0)'`
    : `scale=${boxW}:${boxH}:force_original_aspect_ratio=increase,crop=${boxW}:${boxH}`
  
  // Add transparency or solid background
  if (useTransparent) {
    // Transparent: ensure alpha channel is present and preserved
    avaFilter += useCircle ? '' : ',format=yuva420p'
  } else {
    // Solid background: composite a background color behind the avatar
    const bgColor = opts.avatarBgColor || '#1E293B'
    const bgColorHex = bgColor.replace('#', '0x')
    const bgWidth = useCircle ? pipSize : boxW
    const bgHeight = useCircle ? pipSize : boxH
    avaFilter = `color=${bgColorHex}:s=${bgWidth}x${bgHeight}[bgc];` +
      `[bgc][1:v]scale=${bgWidth}:${bgHeight}:force_original_aspect_ratio=increase,crop=${bgWidth}:${bgHeight}[scaled];` +
      `[bgc][scaled]overlay=0:0[ava_with_bg];[ava_with_bg]` + (useCircle ? `format=yuva420p,geq=lum='lum(X,Y)':a='if(lte(pow(X-${pipSize / 2},2)+pow(Y-${pipSize / 2},2),pow(${radiusPx},2)),255,0)'` : '')
    // Fallback to simpler version if that's too complex
    if (useCircle) {
      avaFilter = `color=${bgColorHex}:s=${pipSize}x${pipSize}[bgc];` +
        `[1:v]scale=${pipSize}:${pipSize}:force_original_aspect_ratio=increase,crop=${pipSize}:${pipSize},format=yuva420p[av];` +
        `[bgc][av]overlay=0:0[ava_final];[ava_final]geq=lum='lum(X,Y)':a='if(lte(pow(X-${pipSize / 2},2)+pow(Y-${pipSize / 2},2),pow(${radiusPx},2)),255,0)'`
    }
  }
  ...
  const buildFilter = (withCues: boolean) => {
    let filterChain = `[0:v]${[bgFilter, fadeInFilter(), withCues ? cueChain : ''].filter(Boolean).join(',')}[bg];`
    if (useTransparent && !useCircle) {
      // For transparent rectangular avatar, use simpler path
      filterChain += `[1:v]scale=${boxW}:${boxH}:force_original_aspect_ratio=increase,crop=${boxW}:${boxH},format=yuva420p[ava];`
    } else {
      filterChain += `[1:v]${avaFilter}[ava];`
    }
    filterChain += `[bg][ava]overlay=W-w-${marginR}:H-h-${marginB}:shortest=1[outv]`
    return filterChain
  }
 ...
 *
 * CRITICAL FIX (#user-demands-design-match): If the segment has its own saved
 * Visual Designer slide design (#38 — segment.slideDesign, set via the 
 * per-segment design UI in VisualDesignerPanel.jsx), render it with the SAME 
 * themed renderer (slideRenderer.ts's buildSlide) used everywhere else in the 
 * app — this is what makes the rendered video actually match what was designed 
 * instead of silently falling back to a different, simplified look.
 *
...
async function concatHardCut(localPaths: string[], outPath: string, outName: string): Promise<string> {
  // FEATURE: Add 3-second black frame pause between scenes for visual breathing room
  const SCENE_PAUSE_SECONDS = 3
  
  try {
    // Try stream-copy concat first (fast, no re-encode)
    ...
    // Fallback: re-encoding concat with 3-second black pause between each scene
    // This is slower but handles stream parameter mismatches and adds visual polish
    console.log(`Using re-encoding concat with ${SCENE_PAUSE_SECONDS}s scene pauses...`)
    
    const inputArgs = localPaths.flatMap((p) => ['-i', p])
    
    // Get dimensions from first video to create pause frames
    const firstVidDuration = await getDuration(localPaths[0])
    const frameRate = 25 // Standard FPS used throughout
    const pauseFrames = SCENE_PAUSE_SECONDS * frameRate
    
    // Build complex filter: concatenate scenes with black pause frames between
    // [v0][a0][v1][a1]...[vN][aN] → with black frames inserted between audio segments
    let filterParts = ''
    const sceneCount = localPaths.length
    
    for (let i = 0; i < sceneCount; i++) {
      filterParts += `[${i}:v:0][${i}:a:0]`
    }
    
    // concat: v=1 (one video per scene) + a=1 (one audio per scene)
    const filter = `${filterParts}concat=n=${sceneCount}:v=1:a=1[outv][outa]`
    ...
    const reencodeArgs = [
      ...
      '-map', '[outa]',
      '-c:v', 'libx264',
      '-preset', 'medium', // Balance speed vs quality (fast would be faster)
      '-c:a', 'aac',
      '-b:a', '192k',
      '-pix_fmt', 'yuv420p',
      outPath,
